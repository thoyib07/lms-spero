...
<div class="form-input-content">
  <div class="card login-form mb-0">
      <div class="card-body pt-5">
          ...
          <form action="{{ route('post-create-step-two') }}" method="POST">
            @csrf
              <label>Agensi</label>
              ...
              <div class="form-row">
                <div class="form-group col-md-6">
                  ...
                  <input type="number" name="nib" value="{{ $agensi->nib ?? '' }}" class="form-control" placeholder="Nomor Induk Berusaha">
              ...
              <div class="form-group">
                <label>Alamat</label>
                <textarea name="alamat" class="form-control h-150px" rows="4" id="comment" placeholder="Alamat">{{ $agensi->alamat ?? '' }}</textarea>
                <span class="text-danger">@error('alamat'){{ $message }}@enderror</span>
              </div>
              <div class="form-row">
                <div class="form-group col-md-6">
                  <label>Email</label>
                  <input type="email" name="email" value="{{ $agensi->email ?? '' }}" class="form-control" placeholder="Email">
                  <span class="text-danger">@error('email'){{ $message }}@enderror</span>
                </div>
                <div class="form-group col-md-6">
                  <label>Password</label>
                  <input type="text" name="password" value="{{ $agensi->password ?? '' }}" class="form-control" placeholder="Password">
                  <span class="text-danger">@error('password'){{ $message }}@enderror</span>
                </div>
              </div>
              <a href="{{ route('create-step-one') }}" class="btn btn-dark">Back</a>
